...
### GameRepository
Abaixo temos um método em Java que utiliza a anotação `@Query` para definir uma consulta SQL nativa np banco de dados. Ele é usado em um contexto de acesso aos dados do nosso `SGBD`.

A consulta SQL retorna uma lista de objetos do tipo `GameMinProjection`. Essa consulta é composta por várias cláusulas SQL.

A cláusula `SELECT` especifica as colunas que serão retornadas na consulta. Nesse caso, foram selecionadas as colunas `id`, `title`, `game_year`, `img_url`, `short_description` e `position` de duas tabelas: `tb_game` e `tb_belonging`. As colunas selecionadas estão sendo renomeadas usando o operador `AS` para os nomes `gameYear`, `imgUrl`, `shortDescription` e `position`.

A cláusula `FROM` especifica a tabela principal da consulta, que é `tb_game`.

A cláusula `INNER JOIN` é usada para combinar registros entre duas tabelas, nesse caso `tb_game` e `tb_belonging`. A condição de junção é definida pela expressão `tb_game.id = tb_belonging.game_id`, que relaciona a coluna `id` da tabela `tb_game` com a coluna `game_id` da tabela `tb_belonging`.

A cláusula `WHERE` é usada para filtrar os resultados da consulta. Nesse caso, a condição é `tb_belonging.list_id = :listId`, onde `:listId` é um parâmetro que será fornecido durante a execução da consulta. Essa condição filtra os registros onde o valor da coluna `list_id` da tabela `tb_belonging` é igual ao valor do parâmetro `listId`.

A cláusula `ORDER BY` é usada para ordenar os resultados da consulta com base na coluna `position` da tabela `tb_belonging`. Os resultados serão retornados em ordem crescente de valor da coluna `position`.

No geral, esse código define um método de consulta que retorna uma lista de objetos `GameMinProjection` com informações de jogos de acordo com o `listId` fornecido.
...
É um aquivo com scripts `SQL` que usamos para popular as tabelas sem ter a necessidade do uso do `Flyway`
